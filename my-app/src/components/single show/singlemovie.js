...
const MovieDetail = () => {
    ...
    useEffect(() => {
        ...
        axios.get(`http://localhost:8000/api/user_movies/`, {
            headers: {
    ...
    const handleStatusToggle = () => {
        ...
        axios.patch(`http://localhost:8000/api/movies/${id}/status/`, { status: newStatus }, {
            headers: {
                'Authorization': `Token ${localStorage.getItem('token')}`,
    ...
    const handleUserListToggle = () => {
        const url = `http://localhost:8000/api/user_movies/`;
        const data = { movie: id };
        const headers = {
            'Authorization': `Token ${localStorage.getItem('token')}`,
            'Content-Type': 'application/json',
